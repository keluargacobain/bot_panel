...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/keluargacobain/permission/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/keluargacobain/permission/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/keluargacobain/permission/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
        ...
        (
            ...
            if [[ ${message_reply_to_message_message_id[$id]} ]]; then
                ...
                '👤 CREATE USER 👤\n\nUsername:')
                    ...
                    [[ "$sizemax" -gt '10' ]] && {
                        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                            --text "$(echo -e "⛔ Use no maximum 10 characters [EX: ajikcobain]")" \
                            --parse_mode html
                        >$CAD_ARQ
                        break
                    }
